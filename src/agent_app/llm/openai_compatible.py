...
class OpenAICompatibleClient(BaseLLMClient):
    ...
    def _headers(self) -> dict[str, str]:
        ...
        return {
            self.api_key_header.strip(): self.api_key.strip(),
        }
    ...
    async def chat(self, messages: list[Message], temperature: float, max_tokens: int | None, tools: list[dict] | None = None) -> LLMResponse:
        ...
        logger.info("Sending chat completion request: model={}, messages={}, tools={}", self.model, len(messages), bool(tools))

        raw = await self._request_json("POST", f"{self.base_url}/chat/completions", json=payload)
